...
t5s5			cp $06				;change panning if fx byte was $08
			...
			ld a,LPON			;set output to center
			jr t5s54
t5s51			dec a				
			jr nz,t5s52			;if fx byte = 1
			ld a,LPL			;set output to left ch
			jr t5s54
t5s52			dec a				;if fx byte = 2
			jr nz,t5s53
			ld a,LPR			;set output to right ch
			jr t5s54
t5s53			dec a				;if fx byte = 3
			jr nz,t5sndlp
			ld a,LPL			;set ouput ch1 to left ch
			ld (tout1),a
			ld a,LPR			;set output ch2 to right ch
			jr t5s55
...
outswt	.EQU $+1				
ecsk2			ld a,LPOFF		;7		;self-modifying
t5play			push bc			;11
			ld b,a			;4
			ld c,LPOFF		;7
			out (0),a		;11
