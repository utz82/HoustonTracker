...
mhxLeft		ld a,c					;check if we're at the first digit
			...
			jr nz,mhxLnos
			ld de,(pencol)		;if yes, update pencol
			ld a,e
			sub 14
			ld e,a
			ld (pencol),de
...
mhxLlp		inc hl					;calculate the pointer position
			djnz mhxLlp
mhxLx0		ld de,(pencol)		;look up current cursor position
			ld a,e
			sub 8					;shift left 8 pixels...
			ld e,a					;...because pencol points to the position after the cursor right now
			jr mhxxs3				;let the "key Right" routine do the rest
...
mhxRlp		inc hl					;calculate pointer position
			djnz mhxRlp				
			ld de,(pencol)		;pencol already points to the right position
...
mhxcmsk2	ld (mhxccurr),a		;update current mode pointer
			...
			;ld (pencol),de	;and print whatever needs to be printed
			;ld b,3
			call dlmde
			call kdelay
			pop hl
