...
scrlup		ld hl,(sngpnt)			;check if we're at the last line of song data
			...
			ld hl,plotSScreen+24+72	;chickendude's optimized scrolly
...
scrldown	ld hl,(sngpnt)			;check if we're at the first line of song data
			...
			ld hl,plotSScreen+768-24-72-5	;chickendude's optimized scrolly again
			ld de,plotSScreen+768-24-5	;skip an extra 4 bytes (right side screen)
			ld a,56
grcopy1
			ld bc,8
			lddr
...
backupdsp	ld (sngpnt),hl
			ld hl,saferam2
			bcall(_savedisp)	;copy current screen to plotSScreen
			ld bc,768
			ld hl,saferam2
			ld de,plotSScreen
			ldir
...
;************************************************************************
mhand		ld hl,saferam2
			bcall(_savedisp)	;copy current screen to saferam2
			call restdisp			;copy saferam2 to plotSScreen
mhand1		ld hl,plotSScreen + 516 + 48
			ld b,12
			ld a,$ff
...
restdisp	ld bc,768				;copy saferam2 to plotSScreen
			ld hl,saferam2
			ld de,plotSScreen
			ldir
			ret
...
sdata .EQU $
			...
			.BLOCK	7999	;reserve space for song data
