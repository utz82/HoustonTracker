;TIM-5	1-bit music routine by utz 02/2013
...
t5s5		cp $06				;change panning if fx byte was $08
			...
			ld a,$03			;set output to center
			jr t5s54
t5s51		dec a				
			jr nz,t5s52			;if fx byte = 1
			ld a,$01			;set output to left ch
			jr t5s54
t5s52		dec a				;if fx byte = 2
			jr nz,t5s53
			ld a,$02			;set output to right ch
			jr t5s54
t5s53		dec a				;if fx byte = 3
			jr nz,t5sndlp
			ld a,$01			;set ouput ch1 to left ch
			ld (tout1),a
			ld a,$02			;set output ch2 to right ch
			jr t5s55
t5s54		ld (tout1),a		;store output mask ch1
t5s55		ld (tout2),a		;store output mask ch2
			jr t5sndlp
			
t5s6		cp $0c					;retrigger if fx byte was $0e
			jr nz,t5sndlp
t5s61 .EQU $
...
t5s62 .EQU $
			...
			dec a
			jr z,t5revf			;if retrigger counter is now 0, revert code modifications
			ld (intbuf1),a		;otherwise, update retrigger counter
...
outswt	.EQU $+1				
ecsk2		ld a,$00		;7		;self-modifying
t5play		push bc			;11
			ld b,a			;4
			ld c,$00		;7
			out (0),a		;11
